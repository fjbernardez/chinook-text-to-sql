...
def test_removed_artist_demo_routes_return_404() -> None:
...
def test_unsafe_sql_exception_handler_is_registered() -> None:
    ...
    response = client.post("/ask", json={"question": "artists", "max_rows": 50})
...
def test_llm_exception_handler_is_registered() -> None:
    override_service(FakeAskService(error=LlmGenerationError("OpenAI unavailable")))

    response = client.post("/ask", json={"question": "artists", "max_rows": 50})

    assert response.status_code == 503
    assert response.json() == {"detail": "OpenAI unavailable"}
